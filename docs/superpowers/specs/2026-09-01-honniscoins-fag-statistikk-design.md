...
2. **Snitt-innsats regnes kun av oppmøtte, vurderte timer** – medalje ∈
   {`bronse`,`solv`,`gull`}. `'0'` (fravær) og `null` (ikke vurdert) holdes
   **utenfor snittet** (fravær er ikke en innsats-vurdering).
3. **Fravær vises likevel** i medaljefordelingen (Visning 5), så bildet ikke skjules.
   `null` (ikke vurdert) telles ikke i noen visning – det er «mangler data».
...
- `statMedalDistribution(state)` → per fag: andeler `{gull, solv, bronse, fravaer}`
  (fravær = `'0'`), normalisert til 1. `null` ekskluderes fra nevneren.
...
5. **Medaljefordeling per fag** – 100 % stablede stolper (gull/sølv/bronse/fravær)
   med forklaring.
...
### Tidsrom (valgfritt, v1-kandidat)
Standard: **hele historikken**. En enkel periode-velger (Denne måneden / Siste 90 dager
/ Alt) kan legges til øverst og bare filtrere `effortRecords` på dato. Markert som
valgfri – tas med hvis det er lett, ellers utsatt.
...
- `js/app.js` – ny `renderStatistikkTab()` (SVG-bygging som i mockupen, men fra ekte
  ...
  Ingen ny binding-logikk nødvendig utover fane-bytte (og evt. periode-velger).
...
- `'0'`/`null` utenfor snitt, men `'0'` med i fordeling; `null` utenfor begge.
